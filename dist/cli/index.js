...
const ui_1 = require("../bluetooth/ui");
...
program
    ...
    new server_1.DarklineServer(config);
});
program
    .command('connect')
    ...
    const client = new client_1.DarklineClient();
...
program
    ...
    const bluetoothP2P = new bluetooth_1.BluetoothP2PClient(config);
    try {
        await bluetoothP2P.join(nickname);
        // Clear console output and start UI
        console.clear();
        // Create and start the beautiful UI
        const ui = new ui_1.BluetoothUI(bluetoothP2P, nickname);
        ui.start();
    }
    catch (error) {
        console.clear();
...
function setupBluetoothP2PChatInterface(bluetoothP2P) {
    const rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout
    });
