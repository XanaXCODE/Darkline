...
import { User, UserGroup, PERMISSIONS, AuthCredentials } from '../types';
...
export class AuthManager {
  ...
  constructor(serverName: string = 'default') {
  ...
  register(credentials: AuthCredentials): { success: boolean; message: string; group?: UserGroup } {
  ...
  login(credentials: AuthCredentials): { success: boolean; message: string; user?: UserDatabase[string] } {
    const { nickname, password } = credentials;
  ...
  hasPermission(nickname: string, permission: string): boolean {
    ...
    const perm = Object.values(PERMISSIONS).find(p => p.name === permission);
    if (!perm) return false;

    return this.getGroupLevel(user.group) >= this.getGroupLevel(perm.requiredGroup);
  }

  private getGroupLevel(group: UserGroup): number {
    const levels = {
      [UserGroup.BANNED]: -1,
      [UserGroup.GUEST]: 0,
      [UserGroup.USER]: 1,
  ...
  promoteUser(adminNickname: string, targetNickname: string, newGroup: UserGroup): { success: boolean; message: string } {
  ...
  demoteUser(adminNickname: string, targetNickname: string, newGroup: UserGroup): { success: boolean; message: string } {
    const admin = this.users[adminNickname];
    const target = this.users[targetNickname];
  ...
  getAllUsers(): { nickname: string; group: UserGroup; registrationDate: string; lastLoginDate?: string }[] {
  ...
  changePassword(nickname: string, currentPassword: string, newPassword: string): { success: boolean; message: string } {
    const user = this.users[nickname];
    if (!user) {
      return { success: false, message: 'User not found' };
    }
  ...
  deleteUser(adminNickname: string, targetNickname: string): { success: boolean; message: string } {
    const admin = this.users[adminNickname];
    const target = this.users[targetNickname];
    ...
    if (admin.group !== UserGroup.OWNER) {
